...
class DogViewSet(viewsets.ModelViewSet):
    ...
    Методы:
        get_queryset: Возвращает queryset собак, с дополнительными аннотациями в зависимости от действия.
            - Для списка собак (`list`) добавляется аннотация `breed_average_age`
            для расчёта среднего возраста собак той же породы.
            - Для получения одной собаки (`retrieve`) добавляется аннотация `same_breed_count`
            для подсчёта количества собак той же породы.
    """
